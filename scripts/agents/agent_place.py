...
class Agent:
    def __init__(self):
        ...
        self.converter = Converter(grasp_z_offset=Config.grasp_z_offset, place_z_offset=Config.place_z_offset, box=Config.box)
    ...
    def infer(self, images: List[OrthographicImage], method: SelectionMethod) -> Action:
        ...
        if self.successful_grasp_before:
            self.successful_grasp_before = False

            action = self.inference.infer(images, SelectionMethod.Random)
            action.type = 'place'
